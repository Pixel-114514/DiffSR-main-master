...
from models.idm.model.sr3_modules import util as Util
from models.idm.model.sr3_modules.util import *
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def generalized_steps(self, databatch, conditional_input=None, continous=False):
        ...
        img_lr, img_hr = databatch['SR'], databatch['HR']
        ...
        data = {
                'inp': img_lr,
                'coord': hr_coord.to(img_lr.device),
                'cell': cell.to(img_lr.device),
                'gt': img_hr,
                'scaler': torch.from_numpy(np.array([p], dtype=np.float32)).to(img_lr.device)
            }
        ...
        if continous:
            ...
            return ret_img[-1]   
    @torch.no_grad()
    def p_sample_loop(self, databatch, continous=False):
        device = self.betas.device
    ...
    def p_losses(self, databatch, noise=None):
