...
class SwinSR(nn.Module):
    ...
    输入: [B, 32, 32, 3]  其中 3 = [x, y, a(x,y)]
    输出: [B, 64, 64, 1]  高分辨率物理场
    """
    def __init__(
        self,
        input_resolution=32,
        ...
        out_channels=1,
        n_hidden=96,              # 隐藏层维度
        n_layers=3,               # Swin Transformer 层数
        depth=2,                  # 每层包含的 Block 数量
        num_heads=4,              # 注意力头数
        window_size=4,            # 窗口大小
        mlp_ratio=4.,             # MLP 隐藏层倍数
        qkv_bias=True,
        qk_scale=None,
        drop_rate=0.,
        attn_drop_rate=0.,
        ...
        self.layers = nn.ModuleList([
            BasicLayer(
                ...
                drop_path=dpr[i*depth:(i+1)*depth],
                norm_layer=norm_layer
            )
        ...
        if scale_factor == 2:
            ...
            self.upsample = nn.Sequential(
                nn.Linear(n_hidden, n_hidden * 4),  # 扩展通道
                nn.GELU(),
            )
            self.pixel_shuffle = nn.PixelShuffle(2)  # 2x 上采样
        else:
    ...
    def forward(self, x):
        """
        Args:
            x: [B, H, W, C] 其中 C = 3 ([x, y, a(x,y)])
        Returns:
            out: [B, H_out, W_out, 1]
        """
        B, H, W, C = x.shape
        assert H == W == self.input_resolution, f"Input size mismatch: {H}×{W} vs {self.input_resolution}"
        ...
        fx = self.norm(fx)
        fx = self.upsample(fx)  # [B, N, n_hidden*4]
        fx = fx.view(B, H, W, -1)  # [B, H, W, n_hidden*4]
        fx = fx.permute(0, 3, 1, 2)  # [B, n_hidden*4, H, W]
        fx = self.pixel_shuffle(fx)  # [B, n_hidden, 2H, 2W]
        fx = fx.permute(0, 2, 3, 1)  # [B, 2H, 2W, n_hidden]
        H_out, W_out = fx.shape[1], fx.shape[2]
        fx = fx.reshape(B, H_out * W_out, -1)  # [B, N_out, n_hidden]

        out = self.output_proj(fx)  # [B, N_out, 1]
        out = out.view(B, H_out, W_out, self.out_channels)  # [B, 64, 64, 1]
