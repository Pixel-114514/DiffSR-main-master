...
class BaseTrainer:
    ...
    def decode_bchw(self,normalizer, tensor):
        # tensor: (B, C, H, W)
        B, C, H, W = tensor.shape
        # (B, C, H, W) -> (B, H*W, C)
        tensor = tensor.permute(0, 2, 3, 1).reshape(B, -1, C)
        # decode
        tensor = normalizer.decode(tensor)
        # 回到 (B, C, H, W)
        tensor = tensor.view(B, H, W, C).permute(0, 3, 1, 2)
        return tensor


    def visualize_flow_field(self, lr_input, sr_output, hr_gt, save_path, title=''):
        """
        专门用于可视化单通道流场（或其分量/大小）的函数。
        将输入、超分结果、真值和误差并排以热力图形式显示和保存。
        """

        # 1. 将Tensor移动到CPU并转换为Numpy数组
        lr = lr_input.squeeze(0).squeeze(0).cpu().numpy()
        sr = sr_output.squeeze(0).squeeze(0).cpu().numpy()
        hr = hr_gt.squeeze(0).squeeze(0).cpu().numpy()
    ...
    def train_one_epoch(self, model: torch.nn.Module, train_loader, optimizer, criterion, scheduler=None, **kwargs) -> LossRecord:
        ...
        for x, y in train_loader:
            # ✅ 只取 x 的第三个通道（流场），形状变成 [B, H, W, 1]
            x = x[..., 2:].contiguous()
            # 维度变换: [B, H, W, C] -> [B, C, H, W]
            if x.dim() == 4:
            ...
            # 构造符合模型需求的字典
            data_batch = {
                "SR": x.to(self.device_id),  # 输入 (低分辨率/条件)
                "HR": y.to(self.device_id)   # 目标 (高分辨率)
            }
    ...
    def process(self, model: torch.nn.Module, train_loader, valid_loader, test_loader, optimizer, 
        ...
        for epoch in range(start_epoch, epochs):
            ...
            train_loss = self.train_one_epoch(model, train_loader, optimizer, criterion, scheduler,  **kwargs)
            ...
            if (epoch + 1) % self.train_config.get('eval_freq', 1) == 0:
                if self.is_main_process:
                    ...
                    valid_loss = self.evaluate(model, valid_loader, criterion, split="valid",visualize=visualize,num_visuals=num_visuals,epoch=epoch,**kwargs)
        ...
        if self.is_main_process:
            test_loss = self.evaluate(model_to_eval, test_loader, criterion, split="test", **kwargs)
